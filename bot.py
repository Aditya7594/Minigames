...
from utils.token_1 import token
...
from systems.claim import get_claim_handlers, daily
from games.wordhunt import register_handlers as get_wordhunt_handlers
from games.wordle import registers_handlers as get_wordle_handlers
from systems.finder import get_finder_handlers

from systems.bank import bank, store, withdraw, add_credits, blacklist, unblacklist, auto_ban, scan_blacklist
...
async def broadcast(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if target in ["users", "all"]:
            # Fetch normal users
            normal_users_cursor = user_collection.find({}, {"user_id": 1})
            normal_user_ids = set()
            for u in normal_users_cursor:
                if "user_id" in u:
                    normal_user_ids.add(str(u["user_id"]))
            
            # Fetch genshin users
            genshin_users_cursor = genshin_collection.find({}, {"user_id": 1})
            genshin_user_ids = set()
            for u in genshin_users_cursor:
                if "user_id" in u:
                    genshin_user_ids.add(str(u["user_id"]))
            
            count_normal = len(normal_user_ids)
            count_genshin = len(genshin_user_ids)
            
            # Combine unique IDs
            all_user_ids = list(normal_user_ids.union(genshin_user_ids))
        ...
        report_message = (
            ...
            f"📢 <b>Groups Reached:</b> {successful_groups} (Failed: {failed_groups})\n"
            f"👤 <b>Normal Users in DB:</b> {count_normal}\n"
            f"🌠 <b>Genshin Users in DB:</b> {count_genshin}\n"
            f"📨 <b>Unique Users Reached:</b> {successful_users} (Failed: {failed_users})\n"
        )
...
async def give(update: Update, context: CallbackContext) -> None:
    ...
    # Function removed: handle_group_message logic is now split between games/genshin_game.py and systems/level_system.py
...
def main() -> None:
    ...
    # Add WordHunt and Wordle
    for handler in get_wordhunt_handlers(application):
        application.add_handler(handler)
