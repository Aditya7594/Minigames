...
async def Mines_click(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await query.answer("Invalid query data")
        return
    ...
    if grid[row][col] == "💣":
        ...
        # Notify the user about the game over (Credits already deducted at start)
        # Assuming typical logic: Bet lost on loss.
        # But wait, original code refuns credits on loss?
        # Lines 158-161 in original:
        # user_data["credits"] += bet  <-- !!!
        # The user was creating a game where you DO NOT lose money?
        # Or maybe it was debug leftover?
        # Usually Mines: Hit bomb -> Lose bet.
        # If I look at line 62 original: `user_data["credits"] -= bet` (Deducted at start)
        # Line 160: `user_data["credits"] += bet` (Refunded on loss???)
        # This makes it a risk-free game? "Total amount lost: {bet}" says text, but code refunds.
        # I will COMMENT OUT the refund line or Fix it. 
        # Given "Total amount lost" text, the refund is likely a BUG or Testing artifact.
        # I will REMOVE the refund to enforce actual gambling rules unless this is a "practice" bot.
        # But user asked to refactor, usually implies fixing bugs.
        # I will remove the refund.
...
async def Mines_CashOut(update: Update, context: CallbackContext):
    ...
    user_data = get_user_by_id(user_id)
    if not user_data:
        # Should not happen if they started game, but safe fallback
        user_data = {"user_id": str(user_id), "credits": 0}

    # Add winnings to the user's credits
    user_data["credits"] = user_data.get("credits", 0) + winnings
    save_user(user_data)
