...
import os
from utils.db import genshin_collection, users_collection, group_settings_collection
...
ARTIFACTS_FOLDER = "assets/images/artifacts"
...
def get_group_settings(chat_id: int) -> dict:
    ...
    chat_id = str(chat_id) # Ensure consistency with other lookups if they store as string, but here likely int for update.effective_chat.id. 
    # Wait, the original code used chat_id as passed (int) for find_one. But earlier in bot.py it was using str(update.effective_chat.id).
    # Let's check `group_settings_collection` usage in previous files. 
    # In `bot.py`: `group_id = str(update.effective_chat.id)` and `groups_collection.update_one({"group_id": group_id}...)`.
    # In `genshin_game.py` original code: `settings = group_settings.find_one({"chat_id": chat_id})`. 
    # `chat_id` came from `update.effective_chat.id` which is int.
    # However, `update_group_settings` logic used `chat_id` as is.
    # To be safe, let's keep it as is from the caller's perspective or standardize.
    # I'll convert to string to be consistent with user_ids if that's the pattern, but `update.effective_chat.id` is int.
    # Let's inspect `bot.py`'s `get_group_settings` if it exists. Yes, line 140 in bot.py: `group_settings_collection.find_one({"chat_id": str(chat_id)})`.
    # It converts to string! So I should convert to string here too.
    
    chat_id_str = str(chat_id)
    settings = group_settings_collection.find_one({"chat_id": chat_id_str})
    if not settings:
        settings = {
            "chat_id": chat_id_str,
...
async def handle_genshin_group_message(update: Update, context: CallbackContext):
    ...
    logger.debug(f"Processing message - User: {user_id}, Chat: {chat_id}, Type: {message_type}")
    ...
    enabled = settings.get("artifact_enabled", True)
    ...
    now = datetime.now(timezone.utc)
    if not user_data:
        # Create user if not exists (silent creation for tracking)
        user_data = {
            "user_id": user_id,
            "primos": 0,
            "bag": {},
...
async def set_threshold(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text("Usage: /setthreshold <number> (10-100)")
        return
...
async def artifact_settings(update: Update, context: CallbackContext) -> None:
    ...
    message = (
        ...
        f"Messages until next artifact: {max(0, settings.get('artifact_threshold', 50) - message_counts.get(chat_id_str, 0))}\n\n"
        "*Admin Commands:*\n"
...
async def handle_artifact_button(update: Update, context: CallbackContext) -> None:
    ...
    artifact_name = query.data.split("_")[2] # "claim_artifact_NAME" -> splits to ["claim", "artifact", "NAME"]

    # Check if the artifact has already been claimed
    # Using chat_data might be unreliable if bots restart. 
    # But for a simple claim mechanic it works if persisted in memory.
    # A better way is to check the message ID or similar.
    # The original code used context.chat_data[f"artifact_{artifact_name}"]. 
    # Wait, artifact_name is not unique per message! 
    # If multiple Glads are dropped, filtering by name blocks future drops?
    # No, usually logical flow is: Drop -> Button(unique_id?).
    # Here button data is just name. So if 2 same artifacts drop, claiming one might verify against... what?
    # Original code: 
    # artifact_data = context.chat_data.get(f"artifact_{artifact_name}")
    # if not artifact_data or artifact_data.get("claimed", False):
    #     ...
    # This implies there's only one active drop of that name allowed? Or logic was flawed.
    # Since I can't easily fix deep logic without more scope, I'll trust the original intent but ensure it doesn't crash.
    # ACTUALLY: The original code in `send_artifact_reward` DID NOT set `context.chat_data`. 
    # So `artifact_data` would always be None? 
    # The original `send_artifact_reward` just sent the photo.
    # The original `handle_artifact_button` checked `context.chat_data`.
    # Unless `send_artifact_reward` was putting it there? 
    # I viewed `send_artifact_reward` in step 476 lines 355-385. It ONLY does `send_photo`.
    # It DOES NOT set `chat_data`. 
    # So the claim check `if not artifact_data` would likely fail or pass depending on logic.
    # If `not artifact_data` -> returns error?
    # Original: `if not artifact_data or ...: await query.answer("❌ This artifact has already been claimed.")`
    # So it was ALWAYS blocking claims? 
    # Unless `artifact_data` is set somewhere else?
    # This looks like a bug in the original code. 
    # I will FIX it by removing the `chat_data` check for now, or just allowing the claim if valid user.
    # To prevent abuse (spam clicking), we can delete the message after one claim.
    ...
    if artifact_name not in user_data["bag"]["artifacts"]:
        user_data["bag"]["artifacts"][artifact_name] = {"image": ARTIFACTS.get(artifact_name, ""), "count": 1}
    else:
    ...
    # Delete the artifact reward message to prevent multiple claims
    try:
        await context.bot.delete_message(chat_id=chat_id, message_id=query.message.message_id)
    except Exception as e:
        logger.error(f"Failed to delete artifact message: {e}")

def reset_artifact_claimed(context: CallbackContext) -> None:
    """Reset the claimed status of an artifact after a certain period."""
    # This seems unused if we delete the message. Keeping as placeholder.
    pass
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    # Check if this button is for claiming artifact
    if query.data.startswith("claim_artifact_"):
        await handle_artifact_button(update, context)
        return
...
async def primo_leaderboard(update: Update, context: CallbackContext) -> None:
    ...
    for i, user in enumerate(top_users, 1):
        ...
        if not name:
            user_data = users_collection.find_one({"user_id": user.get('user_id')})
            if user_data:
