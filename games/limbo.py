...
from utils.db import users_collection, limbo_games_collection, get_user_by_id, save_user
...
async def limbo(update: Update, context: CallbackContext) -> None:
    ...
    user_data['credits'] = user_data.get('credits', 0) - bet_amount
    save_user(user_data)
...
async def handle_take(update: Update, context: CallbackContext, user_id: str, game_id: str):
    ...
    # If multiplier is 0, user lost (though "take" usually implies they chose to stop, but if the current val was 0...?)
    # Logic in `generate_weighted_multiplier` allows 0. 
    # If 0 happens, user sees 0. If they hit "Next" into 0, they see 0. 
    # If they "Take" on 0, they get 0 winnings? Yes.
...
async def handle_next(update: Update, context: CallbackContext, user_id: str, game_id: str):
    ...
    if limbo_game['current_index'] < 4:
        ...
        # Check if the new multiplier is 0 (Instant Lose)
        if limbo_game['multipliers'][new_index] == 0:
             # If it is 0, should we auto-end or let them "Take" 0?
             # "10% chance to lose everything instantly" comment in weighted gen implies loss.
             # User sees 0. Logic allows "Take" 0 to get 0. 
             # Seems fine.
             pass
             
    else:
        # Should not happen via "Next" button on last index, but fallback
        await handle_take(update, context, user_id, game_id)